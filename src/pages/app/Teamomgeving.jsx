...
function Inrichten({ team, uid, leden, herladen }) {
  ...
  return <section className="tk-kaart to-inrichten">
    ...
    <label>Omgevingspakket<input type="file" accept=".json,application/json" onChange={lees} disabled={bezig} /></label>
    {pakket && <><h3>{pakket.inhoud.titel}</h3><p>{pakket.inhoud.onderdelen.length} onderdelen en {pakket.bestanden.length} documenten.</p></>}
    <label>Tweede begeleider (naast jou)<select value={tweede} onChange={(e) => { setTweede(e.target.value); setAkkoord(false); }} disabled={bezig}><option value="">Kies een bestaand teamlid</option>{leden.filter((l) => l.uid !== uid).map((l) => <option key={l.uid} value={l.uid}>{l.naam || "Teamlid"}</option>)}</select></label>
    <label className="to-akkoord"><input type="checkbox" checked={akkoord} onChange={(e) => setAkkoord(e.target.checked)} disabled={bezig} />Ik publiceer de teamonderdelen en documenten voor {team.teamNaam || "dit team"}. Alleen ik en de gekozen begeleider krijgen toegang tot Beheer & bespreking.</label>
...
function Omgeving({ team, uid, leden, magInrichten }) {
  ...
  return <>
    <header className="to-intro"><span className="to-label">Onze teamomgeving</span><h1>{omgeving.inhoud.titel}</h1><p>{omgeving.inhoud.intro}</p></header>
    <nav className="to-tabs" aria-label="Teamomgeving"><div>{omgeving.inhoud.onderdelen.map((d) => <button key={d.id} aria-current={tab === d.id ? "page" : undefined} onClick={() => { setTab(d.id); setMelding(""); }}>{d.titel}</button>)}<button aria-current={tab === "documenten" ? "page" : undefined} onClick={() => setTab("documenten")}>Documenten & terugblik</button>{omgeving.magBeheer && <button aria-current={tab === "beheer" ? "page" : undefined} onClick={() => setTab("beheer")}>Beheer & bespreking</button>}</div></nav>
    ...
    {tab === "documenten" && <section><h2>Documenten & terugblik</h2><div className="to-documenten">{omgeving.inhoud.documenten.map((d) => <article className="tk-kaart" key={d.id}><span className="to-label">Teamdocument · PDF</span><h3>{d.titel}</h3><p>{d.beschrijving}</p><button className="tk-knop tk-knop-rand" disabled={!!bezig} onClick={() => download(d)}>{bezig === d.id ? "Pdf controleren…" : "Pdf downloaden"}</button></article>)}</div><div className="tk-kaart to-tekst"><Tekst>{omgeving.inhoud.documentContext}</Tekst></div></section>}
    {tab === "beheer" && omgeving.magBeheer && <section className="tk-kaart to-tekst"><h2>Beheer & bespreking</h2><p className="to-privacy">Alleen zichtbaar voor de twee aangewezen begeleiders. Andere teamleden kunnen deze inhoud ook niet rechtstreeks opvragen.</p><Tekst>{omgeving.beheer?.tekst}</Tekst><label htmlFor="bespreeknotities">Bespreeknotities</label><textarea id="bespreeknotities" maxLength={20000} rows={8} value={notities} onChange={(e) => setNotities(e.target.value)} /><button className="tk-knop" disabled={!!bezig} onClick={bewaar}>{bezig === "notities" ? "Opslaan…" : "Notities opslaan"}</button></section>}
